...
public class IconButtonUI extends BasicButtonUI {
	...
	private void prepareIcons(AbstractButton b) {
		...
		if (this.scalingValue > 0) {
			baseImage = getScaledImage(baseImage, (int) (baseIcon
					.getIconWidth() * this.scalingValue), (int) (baseIcon
					.getIconHeight() * this.scalingValue));
	...
	private Image getImage(Icon icon) {
		if (icon instanceof ImageIcon) {
			...
			BufferedImage buffer = new BufferedImage(icon.getIconWidth(), icon
	 ...
	 */
	private Image changeBrightness(Image img, final boolean brighten,
			final int percent) {
		ImageFilter filter = new RGBImageFilter() {
			...
			public int filterRGB(int x, int y, int rgb) {
				return (rgb & 0xff000000) | (filter(rgb >> 16) << 16)
						| (filter(rgb >> 8) << 8) | (filter(rgb));
			}
