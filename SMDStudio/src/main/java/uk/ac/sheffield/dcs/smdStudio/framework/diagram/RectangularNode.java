...
public abstract class RectangularNode extends AbstractNode {
	...
	/**
	 * Snaps the bounds of this rectangular node so that it has the node
	 * position as top left corner and the desired width and height, snapped to
	 * the given grid.
	 * 
	 * @param g
	 *            the grid to snap to
	...
	public void snapBounds(Grid g, double width, double height) {
		bounds.setFrame(getLocation().getX(), getLocation().getY(), width,
				height);
		g.snap(bounds);
	}
