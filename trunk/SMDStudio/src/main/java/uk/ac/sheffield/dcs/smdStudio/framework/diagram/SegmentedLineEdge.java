...
import uk.ac.sheffield.dcs.smdStudio.framework.util.ShortestPathConverter;
...
public abstract class SegmentedLineEdge extends ShapeEdge {
	...
	public void draw(Graphics2D g2) {
		...
		startArrowHead.draw(g2, (Point2D) points.get(1), (Point2D) points
				.get(0));
	 ...
	 *            the graphics context
	 * @param p
	 *            an endpoint of the segment along which to draw the string
	 * @param q
	 *            the other endpoint of the segment along which to draw the
	 *            string
	 * @param s
	 *            the string to draw
	 * @param center
	 *            true if the string should be centered along the segment
	 */
	private void drawString(Graphics2D g2, Point2D p, Point2D q,
			ArrowHead arrow, String s, boolean center) {
	...
	private static Rectangle2D getStringBounds(Graphics2D g2, Point2D p,
			Point2D q, ArrowHead arrow, String s, boolean center) {
		if (g2 == null)
		...
		return new Rectangle2D.Double(a.getX(), a.getY(), d.getWidth(), d
				.getHeight());
	}
	...
	public Rectangle2D getBounds(Graphics2D g2) {
		...
		r.add(getStringBounds(g2, (Point2D) points.get(1), (Point2D) points
				.get(0), startArrowHead, startLabel, false));
	...
	public Shape getShape() {
		...
		path.append(endArrowHead.getPath((Point2D) points
				.get(points.size() - 2), (Point2D) points
				.get(points.size() - 1)), false);
	...
	public Line2D getConnectionPoints() {
		ArrayList<Point2D> points = getPoints();
		return new Line2D.Double((Point2D) points.get(0), (Point2D) points
				.get(points.size() - 1));
	}
