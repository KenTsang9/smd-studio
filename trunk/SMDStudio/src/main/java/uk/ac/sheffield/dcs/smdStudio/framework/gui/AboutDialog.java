...
import uk.ac.sheffield.dcs.smdStudio.framework.swingextension.VerticalAutoScrollPane;
...
public class AboutDialog extends JDialog {
	...
	private JPanel getButtonPanel() {
		...
		c.gridx = 1;
		c.gridy = 0;
		JButton closeButton = new JButton(resourceBundle
				.getString("dialog.button.label"));
	...
	private JPanel getSystemInfoPanel() {
		if (this.systemInfoPanel == null) {
			...
			AboutTableModel tableModel = new AboutTableModel(new String[] {
					resourceBundle.getString("systeminfo.col1"),
					resourceBundle.getString("systeminfo.col2") });
			try {
		...
		this.nextButton.setText(resourceBundle
				.getString("dialog.button.show_version"));
	...
	private JPanel getAuthorsPanel() {
		if (this.authorsPanel == null) {
			String imagePath = resourceBundle.getString("authors.banner");
			JLabel image = new JLabel(new ImageIcon(getClass().getResource(
					imagePath)));
			JTextPane tp = new JTextPane();
			try {
				URL url = getClass().getResource(
						resourceBundle.getString("authors.file"));
				tp.setPage(url);
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		this.nextButton.setText(resourceBundle
				.getString("dialog.button.show_license"));
		removeActionListener(this.nextButton);
		this.nextButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
	...
	private JPanel getLicensePanel() {
		if (this.licensePanel == null) {
			...
			try {
				URL url = getClass().getResource(
						resourceBundle.getString("license.file"));
				tp.setPage(url);
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		this.nextButton.setText(resourceBundle
				.getString("dialog.button.show_systeminfo"));
		removeActionListener(this.nextButton);
		this.nextButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
	...
	private JPanel getVersionPanel() {
		if (this.versionPanel == null) {
			...
			JLabel image = new JLabel(new ImageIcon(getClass().getResource(
					imagePath)));
			JLabel text = new JLabel(resourceBundle
					.getString("app.version.text"));
		...
		this.nextButton.setText(resourceBundle
				.getString("dialog.button.show_authors"));
		removeActionListener(this.nextButton);
		this.nextButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
