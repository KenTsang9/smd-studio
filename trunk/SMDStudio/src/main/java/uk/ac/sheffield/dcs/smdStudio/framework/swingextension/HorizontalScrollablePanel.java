...
public class HorizontalScrollablePanel extends JPanel {
			...
			this.innerJScrollPane.addComponentListener(new ComponentListener() {
				...
				public void componentResized(ComponentEvent e) {
					...
					if (innerJScrollPane.getWidth() < innerComponent.getWidth()) {
						if (!getLeftButton(innerComponent).isVisible())
							getLeftButton(innerComponent).setVisible(true);
						if (!getRightButton(innerComponent).isVisible())
							getRightButton(innerComponent).setVisible(true);
					}
					// Hide button if necessary
					if (innerJScrollPane.getWidth() > innerComponent.getWidth()) {
						if (getLeftButton(innerComponent).isVisible())
							getLeftButton(innerComponent).setVisible(false);
						if (getRightButton(innerComponent).isVisible())
							getRightButton(innerComponent).setVisible(false);
					}
				}
	...
	private JButton getLeftButton(JComponent innerComponent) {
		if (this.leftButton == null) {
			...
			this.leftButton.addMouseListener(new SlideListener(
					getToolBarJScrollPane(innerComponent)
							.getHorizontalScrollBar(), SLIDE_LENGTH * -1));
		}
	...
	private JButton getRightButton(JComponent innerComponent) {
		if (this.rightButton == null) {
			...
			this.rightButton.addMouseListener(new SlideListener(
					getToolBarJScrollPane(innerComponent)
							.getHorizontalScrollBar(), SLIDE_LENGTH));
		}
	...
	private class ScrollBarSlider extends Thread {
		...
		private void performSlide() {
			...
			while (currentTime < endingTime) {
				...
				long barValue = initialBarValue + (currentTime - initialTime)
						* slideLength / (endingTime - initialTime);
				if ((slideLength < 0 && barValue < bar.getValue())
						|| (slideLength >= 0 && bar.getValue() < barValue)) {
	...
	private class ThreadSleeper {
		...
		/**
		 * Perform a wait() that forces the owner thread to wait until a
		 * notify() sent on this object
		 */
